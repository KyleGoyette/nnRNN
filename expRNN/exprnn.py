...
from expRNN.initialization import henaff_init
...
class ExpRNN(nn.Module):
    def __init__(self, input_size, hidden_size, nonlinearity=modrelu, exponential="exact", skew_initializer=henaff_init,input_initializer=nn.init.xavier_normal_):
        ...
        self.params = [self.input_kernel.weight]
        self.orthogonal_params = [self.log_recurrent_kernel]
        self.ortho = False
        ...
        if nonlinearity:
            try:
                ...
                if isinstance(self.nonlinearity,modrelu):
    ...
    def reset_parameters(self):
        ...
        self.input_initializer(self.input_kernel.weight.data)
